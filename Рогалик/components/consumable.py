...
class Consumable(BaseComponent):
    ...
    def get_action(self, consumer: Actor) -> Optional[ActionOrHandler]:
        """Попытка вернуть действие для этого элемента."""
        return actions.ItemAction(consumer, self.parent)

    def activate(self, action: actions.ItemAction) -> None:
        """Вызов способности предметов.

`action` — это контекст для этой активации.
        """
...
class ConfusionConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        attack_sound = mixer.Sound('conf.mp3')
...
class FireballDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        attack_sound = mixer.Sound('fire.mp3')
        attack_sound.set_volume(1.0)  # Установите нужный уровень громкости
        attack_sound.play()  # Воспроизведение звукового эффекта
        self.consume()
...
class HealingConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if amount_recovered > 0:
            ...
            attack_sound = mixer.Sound('heal.mp3')
            attack_sound.set_volume(1.0)  # Установите нужный уровень громкости
            attack_sound.play()  # Воспроизведение звукового эффекта
            self.consume()
        else:
            raise Impossible("You are completely healthy.")
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            ...
            attack_sound = mixer.Sound('kchay.mp3')
            attack_sound.set_volume(1.0)  # Установите нужный уровень громкости
            attack_sound.play()  # Воспроизведение звукового эффекта
            self.consume()
        else:
